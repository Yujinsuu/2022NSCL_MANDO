...
def TFlight_callback(msg):
	global light
	light = msg.data[0]
	# -1 : None, 0 : green,  1 : red

def traffic_two_flag(now, delay):
	TRAFFIC_TWO_FLAG = 3
	if (now-delay) > 30 and (now-delay) < 50000 and delay > 0 : # 주차할때 멈추면 안되니까 두번째 신호등 올 때 시간을 비교
		x = ((now-delay)+5)%30 # 첫번째 신호등과 두번째 신호등의 관계식
		# x는 두번째 신호등의 주기의 첫번째여야 한다.  
		if(x< 24): # 주황 and red
			TRAFFIC_TWO_FLAG = 3
		elif (x < 30) : # 초록 
			TRAFFIC_TWO_FLAG = 2
		print((TRAFFIC_TWO_FLAG))

	return TRAFFIC_TWO_FLAG


def check_link_speed_flag(my_wp, link_ind) :

	global flag, mode, escape, tf_count, tf_flag, now, delay
	if flag >= 0:
	...
	if flag == 0: tf_flag = 0; tf_count = 0; escape = 0
	...
	#	if escape <= 70: flag =3
	#	elif escape <= 370:
	#		if light == 4: tf_flag = 1; delay=time.time() - 7 #### camera delay 
	#		if tf_flag == 1: tf_count += 1
	#		if tf_count > 170: link_ind += 1 
	#	else: link_ind += 1
	#
	#	#flag = traffic_two_flag(now,delay)
	...
	if mode == 'T': 
		...
		escape += 1
		if escape <= 50: flag =3
		elif tf_count >= 30: link_ind +=1
		elif escape <= 150:
			if light == 1: #red
				flag = 3
			elif light == 0: link_ind += 1 #green
			else: tf_count+=1
		else: link_ind += 1
...
if __name__ == "__main__":
	...
	while not rospy.is_shutdown():
		...
		print("link num : "+ str(link_ind)+ ", my wp : "+ str(my_wp) + ", speed : "+str(now-delay) + ", flag: "+str(flag))
