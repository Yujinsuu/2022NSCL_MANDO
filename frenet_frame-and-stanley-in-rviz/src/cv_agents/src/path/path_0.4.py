...
def traffic_two_flag(now, delay):
	...
	if (now-delay) > 30 and (now-delay) < 50000 and delay > 0 : # 주차할때 멈추면 안되니까 두번째 신호등 올 때 시간을 비교
		x = (now-delay)%30 # 첫번째 신호등과 두번째 신호등의 관계식
...
def check_link_speed_flag(my_wp, link_ind) :
	...
	if mode == 'T': 
		...
		if escape <= 70: flag =3
			...
			now=time.time()
			if light == 4: tf_flag = 1; delay=time.time() - 5 #### camera delay 
			if tf_flag == 1: tf_count += 1
			if tf_count > 180: link_ind += 1 
		else: link_ind += 1
	...
	elif mode == 'R': target_speed = use_map.target_speed[2]
